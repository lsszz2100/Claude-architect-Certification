# Claude Architect 자격증 준비 가이드

<p align="center">
  <img src="assets/cover.png" alt="Claude Certified Architect 자격증 준비 가이드 표지" width="300">
</p>

<p align="center">초급자부터 합격까지 — 실무 중심으로 정리한 학습서</p>

---

## 바로 시작하기

> 이 책은 TOC.md(목차)를 통해 순서대로 읽도록 설계되어 있습니다.

**[목차 바로가기 →](https://github.com/lsszz2100/Claude-architect-Certification/blob/main/TOC.md)**

또는 첫 페이지부터 읽기:  
[이 책을 읽는 방법 →](https://github.com/lsszz2100/Claude-architect-Certification/blob/main/pages/00_how_to_read.md)

---

## 이 책에 대하여

이 책은 Claude Certified Architect – Foundations 자격증 취득을 목표로 하는 모든 분들을 위해 쓰여졌습니다.

AI가 단순한 도구를 넘어 생각하고 행동하는 에이전트로 진화하고 있는 지금, Claude를 활용한 아키텍처 설계 능력은 개발자와 솔루션 아키텍트에게 필수적인 역량이 되었습니다.

이 책은 단순히 시험을 통과하기 위한 암기 자료가 아닙니다. 실제 프로덕션 환경에서 Claude를 활용하는 방법, 멀티에이전트 시스템을 설계하는 원리, MCP를 통해 외부 세계와 연결하는 기술을 처음부터 차근차근 이해할 수 있도록 설계했습니다.

---

## 시험 핵심 정보

| 항목 | 내용 |
|------|------|
| 자격증명 | Claude Certified Architect – Foundations |
| 문제 형식 | 4지선다 객관식 (오답 감점 없음) |
| 합격 점수 | 720 / 1000 (scaled score) |
| 시나리오 수 | 6개 중 4개 랜덤 출제 |
| 응시 등록 | [응시 신청 페이지](https://anthropic.skilljar.com/claude-certified-architect-foundations-access-request) |

---

## 책의 구성

| Part | 내용 | 챕터 |
|------|------|------|
| Part 1 | 기초 — Claude와 AI 이해 | Chapter 01~03 |
| Part 2 | Domain 1: Agentic Architecture (27%) | Chapter 04~06 |
| Part 3 | Domain 2: Tool Design & MCP (18%) | Chapter 07~08 |
| Part 4 | Domain 3: Claude Code (20%) | Chapter 09~11 |
| Part 5 | Domain 4: Prompt Engineering (20%) | Chapter 12~14 |
| Part 6 | Domain 5: Context Management (15%) | Chapter 15~16 |
| Part 7 | 실전 시나리오 | Chapter 17~22 |
| Part 8 | 실전 문제 풀이 | Chapter 23~26 |
| Part 9 | 시험 전략과 마무리 | Chapter 27~31 |
| 부록 | 용어사전, 치트시트, 코드 모음 | A~D |

전체 목차는 [TOC.md](https://github.com/lsszz2100/Claude-architect-Certification/blob/main/TOC.md)를 참고하세요.

---

## 저자 및 연락처

이 책은 Claude API, Agent SDK, Claude Code, MCP의 공식 문서와 시험 가이드를 분석하여, 초급자가 실무 수준까지 성장할 수 있도록 구성되었습니다.

| 채널 | 링크 |
|------|------|
| 📧 이메일 | [leemanrank@gmail.com](mailto:leemanrank@gmail.com) |
| 💬 인공지능 정보공유 단톡방 | [카카오톡 오픈채팅 참여하기](https://open.kakao.com/o/s4OEqBai) |

---

*이 책은 Claude Certified Architect – Foundations Exam Guide v0.1 (Feb 10, 2025)을 기반으로 작성되었습니다.*
